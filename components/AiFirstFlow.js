...
import { AI_TOOLS, getQuestions } from "@/lib/task-ai-matrix";
...
export default function AiFirstFlow() {
  ...
  // 1. adım: kullanıcı yazdığını gönderince, hangi kategoriye girdiğini
  // arka planda tespit ediyoruz — kullanıcı hiçbir kategori seçmiyor.
    ...
    <div className="bg-[#1C2128] border border-[#2A2F38] rounded-xl p-4">
      ...
      {step === 2 && (
        <div>
          ...
          >
            <ArrowLeft size={13} />{" "}
            <span style={{ color: selectedAI?.color }}>{selectedAI?.name}</span> seçildi, değiştir
          </button>
          ...
          <div className="flex flex-col gap-3">
            <div>
              ...
              <textarea
                ...
                onChange={(e) => {
                  ...
                  // Metni değiştirince eski kategori/sorular geçersiz olur.
            ...
            {/* Henüz kategori tespit edilmediyse: "İlerle" butonu */}
            {!taskId && (
              <button
              ...
              >
                {isClassifying ? (
                  <>
                    <Loader2 size={16} className="animate-spin" /> anlaşılıyor...
                  </>
                ) : (
                  <>
            ...
            {/* Kategori tespit edildiyse: ek sorular + üret butonu */}
            {taskId && (
              <>
                {questions.map((q) => (
                  <div key={q.id}>
                ...
                >
                  {loading ? (
                    ...
                    <>
                      Prompt oluştur <ArrowRight size={16} />
                    </>
                  )}
                </button>
              </>
            ...
            <div className="mt-4 bg-[#14171C] border border-[#4ADEDE]/30 rounded-lg p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                >
                  {copied ? (
                    <>
                      <Check size={13} className="text-[#4ADEDE]" /> kopyalandı
                    </>
                  ) : (
                    <>
